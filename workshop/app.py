...
cities = {
    "Brno": {
        "wiki_pages": ["Brno", "Pivovar Starobrno"],
        "web_pages": [
...
group_types = ["Family with Kids", "Group of nerdy Python Developers", "Bachelor party"]
...
def generate_itinerary(city: str, group_type, start_time, end_time):
    ...
    if index_dir_name.exists():
        ...
        places_nodes = [
            TextNode(text=f"{p.name} - {p.description}") for p in all_attractions
        ]
    ...
    query_engine_tools = [
        RetrieverTool(
            places_index.as_retriever(similarity_top_k=10),
...
Look to wikipedia for more ideas if needed.
    ...
    agent = OpenAIAgent.from_tools(
        query_engine_tools, llm=llm, verbose=True, max_iterations=100
    )
...
def main():
    ...
    city = st.selectbox("Choose the city for your trip:", cities.keys(), index=0)

    group_type = st.radio("Who is traveling?", group_types)
    ...
    end_time = st.selectbox(
        "Select the end time of your visit:", [f"{i}:00" for i in range(24)], index=17
    )
    if st.button("Generate Trip Itinerary"):
        itinerary = generate_itinerary(city, group_type, start_time, end_time)

        st.subheader(
            f"Travel Itinerary for *{group_type}* to *{city}* ", divider="rainbow"
        )
